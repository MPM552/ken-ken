...
export function generatePuzzle(size, difficulty = "easy") {
  ...
  for (let i = 0; i < MAX_RETRY_COUNT; i++) {
    ...
    console.log(
      `Generation attempts: ${i + 1}, Solutions found: ${solutionCount}`,
    );
...
function partitionIntoCages(size, settings) {
  const assigned = Array.from({ length: size }, () =>
  ...
  for (const startCell of allCells) {
    ...
    let cageSize = weightedCageSize(
      maxCageSize,
      singleCellCount,
      maxSingleCells,
      settings.twoCellWeight,
    );
